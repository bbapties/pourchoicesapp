  ...
  if (user === null) {
    return (
      <div className="min-h-full">
        ...
        <p className="text-center text-sm text-cream-mute px-6 py-16">{own ? "Sign in to see your profile." : "No one by that name."}</p>
      </div>
    );
  }

  return (
    <div className="bg-panel min-h-full pb-6">
      <TitleBar
        ...
        <div className="relative shrink-0">
          ...
          {own && (
            <button
              ...
              className="absolute right-0.5 bottom-0.5 w-7 h-7 rounded-full bg-brass border-2 border-cream flex items-center justify-center text-engrave"
        ...
        <div className="flex-1 flex flex-col items-center gap-2.5">
          {own ? (
            <button
              ...
              className="h-10 px-[22px] rounded-full border border-brass-line text-sm font-semibold text-cream bg-panel"
            >
              Edit profile
            </button>
          ) : (
            <div className="flex items-center gap-2.5">
              {rel?.muted ? (
                <button type="button" onClick={handleUnmute} className="h-10 px-[24px] rounded-full border border-edge text-sm font-semibold text-cream-mute bg-panel">
                  Muted · Unmute
                </button>
              ) : (
                <>
                  <button
                    ...
                    className={`h-10 px-[30px] rounded-full text-sm font-semibold disabled:opacity-50 ${rel?.following ? "border border-brass-line text-cream bg-panel" : "text-cream"}`}
                    style={rel?.following ? undefined : { backgroundColor: "#bd9436" }}
                  ...
                  <button
                    ...
                    className="w-10 h-10 rounded-full border border-brass-line bg-panel flex items-center justify-center disabled:opacity-50"
                    aria-label="Notification settings for this person"
                    data-coach="user.bell"
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill={rel?.notifyKinds.length ? "#f6ecd9" : "none"} stroke="#f6ecd9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.7 21a2 2 0 0 1-3.4 0" /></svg>
                  </button>
                </>
              )}
            </div>
          )}
          <div className="text-[13px] text-cream-mute text-center leading-relaxed">
            <div>
              <button type="button" onClick={() => setPeople("followers")} className="underline-offset-2 hover:underline"><b className="font-semibold text-cream">{stats?.followers ?? "–"}</b> followers</button>
              {" · "}
              <button type="button" onClick={() => setPeople("following")} className="underline-offset-2 hover:underline"><b className="font-semibold text-cream">{stats?.following ?? "–"}</b> following</button>
            </div>
            <div><b className="font-semibold text-cream">{stats?.tried ?? "–"}</b> tried · <b className="font-semibold text-cream">{stats?.blinds ?? "–"}</b> blinds</div>
          </div>
        </div>
      </div>
              ...
              <div key={b.bottleId} className="flex flex-col items-center" style={{ width: win ? 110 : 96 }}>
                <div className="flex items-end" style={{ height: 124 }}>
                  {b.imageUrl ? (
                    ...
                    <div style={{ height: h, width: h * 0.32 }} className="rounded-sm bg-panel-3" />
                  )}
                </div>
                <div className={`h-4 mt-2 text-[11px] tracking-[.1em] ${win ? "font-semibold text-cream" : "text-cream-faint"}`}>{win ? 1 : i === 0 ? 2 : 3}</div>
                <div className={`h-8 leading-4 ${win ? "text-[13px]" : "text-xs"} font-semibold text-cream text-center max-w-full`} style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{b.name}</div>
        ...
        <div className="pt-1">
          ...
          {hasMore && (
            <div className="p-4 text-center">
              <button type="button" onClick={loadMore} className="text-sm text-cream-mute underline">Load more</button>
            </div>
          )}
        </div>
      )}
      ...
      {askPush && user && (
        <div className="fixed inset-0 z-40 flex items-end" onClick={() => answerPush(false)}>
          ...
          <div className="relative w-full pc-leather rounded-t-2xl px-5 pt-3 pb-8" onClick={(e) => e.stopPropagation()}>
            <div className="w-9 h-1 rounded bg-panel-3 mx-auto mb-4" />
            <div className="text-base font-semibold text-cream mb-1">Get a push when @{user.username} pours?</div>
            <p className="text-sm text-cream-mute mb-4">You can fine-tune which events from the bell on their page.</p>
            <div className="flex gap-2.5">
              <button type="button" onClick={() => answerPush(true)} className="flex-1 h-11 rounded-full text-sm font-semibold text-cream" style={{ backgroundColor: "#bd9436" }}>Yes</button>
              <button type="button" onClick={() => answerPush(false)} className="flex-1 h-11 rounded-full text-sm font-semibold border border-brass-line text-cream bg-panel">Not now</button>
            </div>
          </div>
        </div>
      )}
      ...
      {manualOpen && (
        <div className="fixed inset-0 z-40 flex items-end" onClick={() => setManualOpen(false)}>
          ...
          <div className="relative w-full pc-leather rounded-t-2xl px-5 pt-3 pb-8" onClick={(e) => e.stopPropagation()}>
            <div className="w-9 h-1 rounded bg-panel-3 mx-auto mb-4" />
            <div className="text-base font-semibold text-cream mb-1">Manual rated</div>
...
function TitleBar({ title, subtitle, back, onBack, gear, onGear }: { title: string; subtitle?: string; back?: boolean; onBack?: () => void; gear?: boolean; onGear?: () => void }) {
  return (
    <div className="sticky top-0 z-20 h-[72px] bg-panel border-b border-brass-line flex flex-col items-center justify-center gap-[3px]" style={{ paddingTop: "env(safe-area-inset-top)" }}>
      {back && (
      ...
      <div className="text-2xl font-semibold text-cream leading-none tracking-[-.01em]">{title}</div>
      {subtitle && <div className="text-xs text-cream-mute">{subtitle}</div>}
      {gear && (
        <button type="button" onClick={onGear} className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center" aria-label="Settings" data-coach="profile.settings">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f6ecd9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.8l.1.1a2 2 0 1 1-2.8 2.8l-.1-.1a1.7 1.7 0 0 0-1.8-.3 1.7 1.7 0 0 0-1 1.5V21a2 2 0 1 1-4 0v-.1a1.7 1.7 0 0 0-1.1-1.5 1.7 1.7 0 0 0-1.8.3l-.1.1a2 2 0 1 1-2.8-2.8l.1-.1a1.7 1.7 0 0 0 .3-1.8 1.7 1.7 0 0 0-1.5-1H3a2 2 0 1 1 0-4h.1a1.7 1.7 0 0 0 1.5-1.1 1.7 1.7 0 0 0-.3-1.8l-.1-.1a2 2 0 1 1 2.8-2.8l.1.1a1.7 1.7 0 0 0 1.8.3H9a1.7 1.7 0 0 0 1-1.5V3a2 2 0 1 1 4 0v.1a1.7 1.7 0 0 0 1 1.5 1.7 1.7 0 0 0 1.8-.3l.1-.1a2 2 0 1 1 2.8 2.8l-.1.1a1.7 1.7 0 0 0-.3 1.8V9a1.7 1.7 0 0 0 1.5 1H21a2 2 0 1 1 0 4h-.1a1.7 1.7 0 0 0-1.5 1z" /></svg>
        </button>
      )}
    </div>
  );
...
function SectionHead({ title, hint }: { title: string; hint?: string }) {
  return (
    <div className="px-4 pt-[18px] pb-1.5 flex items-baseline justify-between">
      <h2 className="text-[11px] font-semibold uppercase tracking-[.14em] text-cream">{title}</h2>
...
function Plate({ children, dashed = false }: { children: React.ReactNode; dashed?: boolean }) {
  return (
    <div className={`mx-4 rounded-lg px-4 py-[18px] text-center text-xs text-cream-mute border ${dashed ? "border-dashed border-edge" : "border-edge"} bg-panel`}>
      {children}
    </div>
  );
}
