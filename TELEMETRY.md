...
- `click` → `home_drink_trim` / `home_scan_trim` — the two shortcuts on Home's top trim (`HomeClient`).
- `click` → `blind_random_lineup` — "Surprise me from my bar" dealt a lineup (`DrinkClient.startMode`).
  `metadata = { count, mode: 'self'|'helper', owned }`.
