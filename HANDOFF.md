...
- **B-08** — signup is thinner than Profile: no `validateUsername`/uniqueness check, `users` insert errors ignored, always `router.replace("/mybar")` → duplicate username can leave an Auth row with no `public.users` row (bounce loop). `src/app/page.tsx` ~97–131.
- **B-07 SHIPPED** (Claude, 2026-08-27): saveTasting retry no longer double-scores Elo — reuse the session on retry + upsert results with `ignoreDuplicates` (existing `(session,winner_bottle,loser_bottle)` unique guard makes it a no-op). App-only, no RPC/schema/trigger change. Verified via rolled-back DB test. See BUGS B-07.
- Then Wave 1b (B-09…B-17) unless Brian reorders.
