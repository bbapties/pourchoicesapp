...
- **Build Story 3.2 — Solo Mode 2 (self-serve) tasting flow** (the core loop). Brian gave a broad go to build all of Phase 3, testing along the way, keeping docs updated. See the full design + story split in the plan file: `C:\Users\whisk\.claude\plans\honestly-we-can-differ-immutable-matsumoto.md`. Order: 3.0 ✅ → 3.1 ✅ → **3.2 solo Mode 2 flow** → 3.3 solo Mode 1 (helper) → 3.4 group → 3.5 trimmings.
- **3.1 DONE (committed local `209d72a`, not pushed).** Manual star "guess" captured in the Have-a-drink flow (post-pour `RatePromptSheet`) + editable on the detail when the bottle's in your bar; Elo numbers are now hidden everywhere (shown as 0-5 stars — `BottleDetailView` "Global rating" + "My rating" rows); once blind-tasted the star is the locked Elo-derived value with the "auto-calculated from your tasting history" message. Files: `src/lib/ratings.ts`, `src/components/StarRatingSlider.tsx`, `src/components/RatePromptSheet.tsx`, `BottleDetailView.tsx`. Verified pour->prompt->save->persist->display live. **The locked/tasted-star display + non-flat star scaling still need a real tasting to exercise — re-check during 3.2.**
- **TESTING NOTE:** the local dev browser session was logged in as **Lakehouse (Brian's real account)**, not the Claude QA account — test artifacts were cleaned up, but **switch to `claude@pourchoicesapp.com` (username "Claude Code Agent", password = the username) before UI testing** to keep test data off Brian's account.
