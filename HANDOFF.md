...
- **Through B-27 DONE.** **B-25** app password min (8); **B-26** forgot-password + `/reset-password` page (⚠ Brian: add the redirect URL in Supabase Auth → URL Configuration, enable the reset email template); **B-27** case-insensitive username unique index. **B-28** (signup reuse `validateUsername`) is already satisfied by the B-08 fix — verify + tick. Next real item after B-28: **B-29+** (see BUGS). `src/app/page.tsx` · `src/app/reset-password/page.tsx` · `sql/b27-username-ci-unique-migration.sql`.
