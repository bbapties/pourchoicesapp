...
- **Auth/signup/profile cluster DONE through B-29** (B-25 password min, B-26 forgot-password + `/reset-password`, B-27 CI username index, B-28 verified via B-08, B-29 delete-user detaches authorship before the auth delete). ⚠ Brian still: add the `/reset-password` redirect URL in Supabase Auth config + enable the reset email template.
- **Next: B-30** — self-serve account delete + data export on Profile. This is a FEATURE, not a quick fix: self-delete needs a path a regular user can call (the admin delete-user route is admin-only → new RPC or a self-delete route), plus a data-export bundle. Its own focused pass. **Then the new cluster B-31…B-46 (Collection/variants/search)** starts with high-priority items (B-31 SKU-vs-variant collection UI, B-32 dup SKU across tabs, B-33 remove-tasted). B-74 (auth id vs public id) is gated.
- **B-13…B-17 SHIPPED**: Search `.or()` escaping + error toast; self-serve confirm wording; collection actions use the visible variant; Add-Back switches to In My Bar; Social ownership race resolved.
- **Wave 0 SHIPPED/resolved (2026-08-27):** B-18 anon users-read locked to authenticated + `email_exists` RPC; B-19 `protect_user_role` trigger; B-20 confirmed already-safe; B-21 service-role env fallback; **B-22 resolved by demoting `claude@`/`grokbuild@` to `user`** (The_Lake_House is now the sole admin; roles snapshotted in `backup_notes_fix.qa_roles`); B-23 tier-1 (tasting_results SELECT+INSERT only); B-24 store-pick RLS + `security_invoker` views. Auth migrations: `sql/b18-b19-auth-hardening-*.sql`, `sql/b23-b24-security-*.sql` (applied to prod, rollbacks included). **⚠ Verify search/My Bar/Social + login still work on the deployed site — B-24 changed the search views to `security_invoker`, and B-18 changed the login email-check path.**
- **Barcode scanner SHIPPED** (Claude, 2026-08-27, off-list per Brian — Phase 8.4 search side): a `ScanLine` icon on the right edge of the Search bar opens a full-screen camera scanner (`BarcodeScannerSheet`, ZXing `@zxing/browser`). On a successful scan, `lookupBottleByBarcode` (`lib/barcode.ts`, matches UPC-A/EAN-13 candidate forms) opens the matching bottle, or jumps to Add Bottle with the barcode pre-filled. `ProvisionalSheet` gained a `barcode` field + `initialBarcode` prop (persists to `bottles.barcode`). New event `click/barcode_scan` + coach `search.barcode`. **On Search AND My Bar** — My Bar's scan opens the matched bottle with real collection status (fetched, since a scanned bottle may not be in the active tab; My Bar now tracks `selectedOwned` in state instead of deriving from the tab), or Add Bottle → opens the new bottle so it can be added to the bar. Camera needs HTTPS (prod ✓); verify on a real phone. Barcode *contract* (normalization/uniqueness) still worth aligning as barcodes get seeded.
