...
- **Wave 1b is DONE** (B-09…B-17 all shipped). **Next is Wave 0 (B-18…B-24) — GATED**: auth/RLS/env/security (anon SELECT on `users`, self-updatable `role`, `delete_user_cascade` admin re-check, service-role env name, weak QA password, Elo farming via raw inserts, client-only store-pick privacy). Several are "confirm on prod first." **Ask Brian before touching any of these.**
- **B-13…B-17 SHIPPED** (Claude, 2026-08-27): Search `.or()` escaping + error toast; self-serve confirm wording; collection actions use the visible variant; Add-Back switches to In My Bar; Social ownership race resolved.
